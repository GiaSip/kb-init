...
> L2 的主体是主题而不是轨迹。**
>
> 2B 据此把这三类**明确推迟**（2B spec §3.5）：两份目标语料上 `available=false`，
> 实现了也验证不了。解除条件是出现一份 `available=true` 的目标语料。
> 代替它们进报告的是一条诚实的说明——「只有 N 篇能确定时间，所以这份报告里
> 没有任何时间轴洞察」。
...
> ⚠️ **2D 实测修订（2026-08-16）：这五节里 v0.1 只填得出一节，且它是降级版。**
>
> 1. **「按稳定性排序」在目标语料上无从算起**——`time_axis.available == false`
>    （日期可解析率 5–6%）。实际实现按篇数排序，产物的导语如实写明这一点。
>    照抄「按稳定性排序」的措辞就是产物撒谎。
> 2. **「领域词汇表」没有上游供料**：L2 产出的洞察全部指向「关注领域」。
>    要出这一节，必须由 2B 新增一条洞察（走人肉 gate），**不能由 2D 自己
>    从关键词汇总**——那样用户审的是一句话，进档案的是另一句话。
> 3. 后三节全部要 L3（2E）。
>
> 因此 2D 实现为**纯管道**：节数由上游有多少料决定，2E 接上后自动多出三节，
> 2D 一行都不用改。另加一节不在本清单里的 **「这份档案的覆盖范围」**——
> 主题只覆盖 kept 的 16–23%，档案不说这句，agent 会把它当成全集。
> 详见 `docs/superpowers/specs/2026-08-16-2d-compile-claude-md-design.md`。
...
| **R14** | ✅ **已实现（2026-08-17）**：进度输出接上了 `embed.py` 里那个一直没人接的钩子；「预估时间」降级为「预先告知量级」——没有可靠依据的预估就是猜。原文：**首次运行不是"秒开"**：用户须先有 `uv`，首跑可能下载 Python + 依赖 + ~90MB 模型，按**分钟**计。这与"对不懂的人可见、10 分钟出 aha"的定位直接冲突 | ①宣传语改为"装好 uv 后一条命令运行，无需自己装 Python"，不吹零安装 ②首次运行体验按一等公民设计（进度、预估时间、下载可见）③GitHub Releases 单二进制列入 v0.2 明确路线——**但 v0.1 不同时维护两套实现** |
| **R15** | **跨平台原生 wheel**（`onnxruntime` 等）是真正的分发风险，不是 uv 本身 | **2026-08-17 实测修订**：查 PyPI 实际 wheel 清单后，原处方的第一项**做不到**——`onnxruntime` 至少从 1.18 起就不发 macOS x86_64 wheel，macOS Intel 是**装不上**而不是"还没验"。真实支持面：Win x64/arm64 ✅ / Linux（glibc ≥ 2.28）✅ / macOS arm64（≥ 14）✅ / **macOS Intel ❌**。已如实写进 README 与 classifiers，CI 矩阵按这个真实面配。⚠️ **依赖侧已验，构建侧仍未验**——CI 配置写好了但从未跑过（仓库还没有远端），两者不是一回事 |
...
| R10 | ~~正式项目名未定~~ | ✅ **已裁决（2026-08-17，贾老师）**：沿用 `kb-init`（PyPI 名可用，全仓库已在用，改名成本为零）。协议裁为 **Apache-2.0** |
