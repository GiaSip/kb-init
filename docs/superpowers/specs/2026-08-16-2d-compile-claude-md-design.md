...
> ⚠️ 四审 #1：「没有回执」有**两种**成因——那文件根本不是我们写的，或者上一次运行
> 在「作废旧回执」与「写新回执」之间失败了。拒绝的动作相同，但**诊断必须分开**：
> 把后者说成「可能是你自己的一篇笔记」是在冤枉用户，会让人不敢删自己的东西。
> 分辨的办法是读那行出处标记——**只为了把话说准，不构成授权**，两条分支照样拒绝。
> 标记可以被复制，它能证明的只有「这看起来像我们写的」。
>
> 这也是不采纳「把作废回执挪回 `os.replace` 之后」的理由：那只是把「冤枉用户的
> 诊断」换成「撒谎的回执」（硬不变量 #4），而诊断可以在诊断层修好。哈希这一条顺带解决了初稿没想到的情形：**用户手工改过档案**——
那是他的编辑，不该被我们无声抹掉。
