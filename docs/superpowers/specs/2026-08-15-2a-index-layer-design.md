...
> **回头条件（写死在此，避免"以后再说"变成"永远不说"）**：若 2B 完成后出现下列任一情况，回到 2A。
>
> ⚠️ **处方映射已于 2026-08-15 修正**。原文写的是"任一触发即加 halo"，这对①②是错配的：
> halo 把 residual 以低置信度挂进**已有**的簇，提高覆盖率但**不产生新主题**。
>
> | 条件 | 病灶 | 对症处方 | 2026-08-15 判定 |
> |---|---|---|---|
> | ① topic 族洞察凑不满 | 主题**数量**不足 | **D′ 过大簇细分** | 曾触发，已由 2A′ 处置 |
> | ② 洞察全部挤在 ≤2 个主题上 | 主题**分布**集中 | **D′ 过大簇细分** | 曾触发（第二份语料字面只有 2 个 group），已由 2A′ 处置 |
> | ③ residual 在第二份**第三方**语料上仍 >70% | 主题**覆盖率**不足 | **C halo 归属** | **not_evaluable** —— 手上两份语料同属一位作者 |
>
> ①的判据是 **topic 族条数**，不是洞察总数——按总数算的话 corpus 族的统计条目会把数量
> 填满，条件永远为假。详见 2B spec §2.2 / §3.1。判定结果随产物落盘在
> `insights.json` 的 `revisit_gate` 里。
