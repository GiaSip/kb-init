...
> 最后更新：2026-08-15（2B 洞察层完成，含前置 2A′ 过大簇细分）
...
⚠️ **开工前先看一眼这个**：四天 86 个 commit 之后，README 承诺的两样门面产物
（可分享的知识报告 / agent 能用的 CLAUDE.md）**一个都还没有**——现在跑完拿到的全是
用户看不见的中间层。排序本身有理由（Codex 判 IR 合同是头号架构风险；DESIGN §4.2 把裁决权
给了档案线，2D 必须先于 2C），但 **2D 的验收标准里要加一条「这东西能不能拿出去给人看」**，
不能只验合同对不对。这个项目的目标是影响力，拿不出东西就没有影响力。

**2D — `compile` → CLAUDE.md**。只收 `insights.md` 里勾选的条目，正文按 ID 从
`insights.json` 取，不信任手改过的文案；`claude_md` 字段已声明每条洞察的去向
（`corpus` 族一律 `null`，不进档案线）。

**2D 的头号分叉（brainstorming 第一个要裁的）**：DESIGN §6 要求 CLAUDE.md 产五节
（关注领域 / 领域词汇表 / 判断偏好 / 已知盲区 / 明确排除项），但 **L2 只供得起前两节**，
后三节都要 L3（2E，需用户自己的 key）。三条路：①先发两节；②调顺序先做 2E；
③2D 只做管道不做内容（按 `claude_md` 字段路由，节数由上游有多少料决定，L3 接上自动多三节）。
倾向③但**无证据**——裁之前先拿两份真实语料的 topic 洞察压成 CLAUDE.md 看看实物。
